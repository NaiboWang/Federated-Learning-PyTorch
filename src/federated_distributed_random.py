...
"""
随机的的从1到num_users中选择frac%的用户训练model，第i个model初始化为第i-1个model训练后的模型

坏处：不能parallel，不够efficient
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for idx in idxs_users:
            # print("Training at user %d/%d, process: %.2f" % (idx,args.num_users,ii/len(idxs_users)))
            # ii = ii + 1
    ...
    torch.save(global_model.state_dict(),'../save/global_model.pkl')
